...
The results of 6 volume data sets were analyzed. These data sets are engine block, nucleon, head, tooth, foot, CT-Knee and MRbrain \cite{website:Roettger_volume_2013} \cite{website:Voreen_datasets_2013}.
There were 30 people participated in the experiment. Among the participants, there were 20 males and 10 females, all aged between 22 and 39 years old. They consist of postgraduates, undergraduates and researchers. Over half of the participants have computer science background. Only one participant reported that he had slight color blindness (red-green). This participant confirmed that he had no problem in distinguishing the colors of features in the experiment.
...
We applied the Spearman's rank correlation to analyze the monotonic association between the mean opinion scores (MOS) and three computational metrics, i.e. visibility-weighted saliency (VWS), feature visibility (FV) and 2D feature saliency (2DFS).
As shown in Figure~\ref{fig:mos_vs_vws}, there are strong positive correlations between MOS and VWS ($ 0.66985 $), and MOS and FS ($ 0.677018 $) respectively. On the other hand, there is a moderate positive correlation between MOS and 2DFS ($ 0.545858 $). Both VWS and FS are more monotonic correlated to MOS than 2DFS, and FV is slightly better than VWS by about $ 0.007 $.

The p-values indicate statistical significance of the associations. In our results, the p-values for all the three cases are very small (less than $2 \times 10^{-5} $). Therefore, we can reject the null hypothesis (there is no association between the two variables) and conclude that there are associations between the three pairs of variables respectively.
...
\begin{figure}
	...
	\begin{minipage}{.33\textwidth}
		...
		\subcaption{ Spearman's $\rho=0.545858 $, P-value$=1.9628 \times 10^{-5} $ }
	\end{minipage}
