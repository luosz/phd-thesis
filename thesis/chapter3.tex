...
The optimizer terminates when further iterations almost do not change the quality of the resulting image.
In our implementation, the optimization terminates when the energy function falls below a predefined threshold or reach a maximum iteration count.
...
Our approach is relatively lightweight, e.g. we observed that the optimization finished within one second on the CT-Knee and VisMale data sets with transfer functions consisting of 24 control points.
...
of tent-like shapes]{Before optimization: CT-Knee rendered with a transfer function consisting 
...
\caption{~}
\end{subfigure}
\caption[After optimization: CT-Knee rendered with the transfer function consisting of tent-like shapes]{The transfer function from Figure~\ref{fig:tf_CT-Knee_spectrum_6} after optimization: (a) Optimized transfer function (b) Optimized output}
