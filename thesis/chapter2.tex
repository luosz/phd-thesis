...
%3D discrete scalar fields (volume data sets) is Marching Cubes \cite{lorensen_marching_1987}.
...
%The development in GPU technology result in powerful parallel processors that enable computation intensive tasks such as ray casting of large volume data sets at interactive rates \cite{beyer_state---art_2015}.
...
%http://web.cse.ohio-state.edu/~crawfis/cis694L/Slides/TextureSlicing.pdf
%http://cs.boisestate.edu/~alark/cs564/lectures/Volume_Visualization.pdf
...
%Volume rendered images are produced by rendering the slices that are most perpendicular to the viewing direction from back to front. When the view is rotated, switching from one stack of slices to another may cause noticeable changes in the volume rendered image \cite{kruger_new_2010}.
...
%In scientific visualization, features of interest are often inner structures of the data sets, e.g. visualizing internal organs in anatomical data sets.
...
%Two-level volume rendering \cite{hauser_two-level_2001} \cite{hadwiger_high-quality_2003} \cite{corcoran_perceptual_2010} is a method of merging several volume rendering techniques into a single rendering. This method is useful when inner structures need to be rendered along with semitransparent outer parts.
%Focus and context \cite{wang_magic_2005} \cite{bruckner_illustrative_2006} \cite{chen_intelligent_2008} techniques reveal the interior of a volume data set in a feature-driven way and also retain the context information.
%Cutaway techniques \cite{burns_feature_2007} \cite{sigg_intelligent_2012} make inner structures clearly visible along with their spatial relation to the surrounding material.
...
%There are other alternative techniques, e.g. exploded views \cite{bruckner_exploded_2006} and structure extrusions \cite{diaz_adaptive_2012}.
% generate cuts on
% volumetric models through the manipulation of a clipping
% plane

% In this way, the user may interactively specify how
% much portion of each visible structure needs to be extruded
% off the cutting plane.

% In
% this paper we present a new editing technique based on the use of clipping planes, direct structure extrusion, and
% illustrative methods, which preserves the context by adapting the extruded region to the structures of interest of the
% volumetric model. 
%an exploded view the object is decomposed into sev-
%eral parts which are displaced so that internal details are visible 
%D{\'i}az et al. \cite{diaz_adaptive_2012}
...
\subcaption{A user-defined opacity transfer function}
\end{minipage}~
\begin{minipage}{.49\textwidth}
\includegraphics[width=1\linewidth]{images/correa_visibility-driven_2009_b}
\subcaption{Match the visibility histogram with the user-defined opacity transfer function}
\end{minipage}
...
%Finding good transfer functions for static volume data itself has proven difficult \cite{pfister_transfer_2001}.
...
Hsieh et al. \cite{hsieh_feature_2013} introduced a particle swarm method for transfer function design. This method maps the frequency of particle occurrences to color and opacity values, and extracts features after the particles finish searching for features in the volume data.
...
Chen and J{\"a}nicke \cite{chen_information-theoretic_2010} presented an information-theoretic framework for visualization. Examining the theoretical aspect of information and its relation to data communication, they interpreted different stages of the visualization pipeline using the taxonomy of information.
...
Predicting salient regions in images \cite{zhao_learning_2013} can be exploited as useful feedback for improving visualizations.
