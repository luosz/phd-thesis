...
Volume visualization is another term for volume rendering, sometimes with emphasis on the realistic aspects of volume rendering.
...
Volume ray casting is a rendering technique that can produce very high quality results.
...
Typical splatting approaches classifies and shades voxels before the projection. Hence, each voxel's footprint is weighted by its color and opacity specified by the transfer function. Projecting these fuzzy color balls leads to a blurry appearance of edges and boundaries of the objects.

Splatting traditionally classifies and shades the voxels prior to projection, and thus each voxel foot-
% print is weighted by the assigned voxel color and opacity. Project-
% ing these fuzzy color balls provides a uniform screen image for
% homogeneous object regions, but leads to a blurry appearance of
% object edges.

%Each projected voxel is represented as a radially symmetric interpolation kernel
%
%We would like to investigate better gradient kernels than the
%Gaussian. However, we are limited by the fact that we have to use
%radially symmetric kernels. Thus we cannot efficiently use the ker-
%nels presented in [2] and [20], but work is underway to expand the
%theory presented in [20] to rotationally symmetric kernels. Another
%promising avenue of research in this respect are the Bessel-Kaiser
%kernels derived by Lewitt [16].

% In splatting, each projected voxel is represented as a
% radially symmetric interpolation kernel, equivalent to a fuzzy ball.
% Projecting such a basis function leaves a fuzzy impression, called a
% footprint or splat, on the screen. Splatting traditionally classifies
% and shades the voxels prior to projection, and thus each voxel foot-
% print is weighted by the assigned voxel color and opacity. Project-
% ing these fuzzy color balls provides a uniform screen image for
% homogeneous object regions, but leads to a blurry appearance of
% object edges. The latter is clearly undesirable, especially when the
% view is zoomed on the object. In this work, we manipulate the ren-
% dering pipeline of splatting by performing the classification and
% shading process after the voxels have been projected onto the
% screen.

\subsection{Shear Warp}
\cite{lacroute_fast_1994}

\subsection{Texture-Based Volume Rendering}
\cite{westermann_accelerated_2001}
...
%We describe a method for computing such
...
%In this paper, we presented a novel quality metric for visu-
%alization based on visual salience. This metric favors visu-
%alizations that guide the observers visual attention to rele-
%vant parts of the image and is generically applicable to all
...
% For the spatiotemporal saliency, we combined the
% spatial saliency with the temporal saliency by simply com-
% puting SAD between temporal gradients of the center and the
% surrounding regions
